...
//  if (ManualMode==LOW and ManualModePrev==HIGH ){
//    microSteps_X=16;
//    setMS123(microSteps_X,'X');
//    stepperSpeedX = microstep_to_manualSpeed(microSteps_X, maxManualSpeedX);
//    
...
//      TargetCurr_X = (Xpos+(-1)*Xneg)*100;
  ...
  if (sendData){
    ...
    /*
    SerialUSB.write(byte(phase_code_lastTrigger%256));SerialUSB.write(byte(phase_code_lastTrigger>>8));          //The phase code is updated by the time handler
    
    CurrPos_X_code = CurrPos_X;  //right sens of the motor
    if (CurrPos_X_code>0){
      SerialUSB.write(byte(int(0)));
      
    }
    else{
      SerialUSB.write(byte(int(1)));
      CurrPos_X_code = -CurrPos_X_code;
      
    }
    SerialUSB.write(byte(CurrPos_X_code>>24));SerialUSB.write(byte(CurrPos_X_code>>16));SerialUSB.write(byte(CurrPos_X_code>>8));SerialUSB.write(byte(CurrPos_X_code%256));    //4 bytes
    ...
    SerialUSB.write(byte(CurrPos_Y_code>>24));SerialUSB.write(byte(CurrPos_Y_code>>16));SerialUSB.write(byte(CurrPos_Y_code>>8));SerialUSB.write(byte(CurrPos_Y_code%256));    //4 bytes

   
    
    if(_EncoderTicks>0) {                        //1 bytes
      SerialUSB.write(byte(int(0)));
      _EncoderTicks_code=_EncoderTicks;}
    else {
      SerialUSB.write(byte(int(1)));
      _EncoderTicks_code=-_EncoderTicks;}
      
    SerialUSB.write(byte(_EncoderTicks_code>>24));SerialUSB.write(byte(_EncoderTicks_code>>16));SerialUSB.write(byte(_EncoderTicks_code>>8));SerialUSB.write(byte(_EncoderTicks_code%256));    //4 bytes
    
    SerialUSB.write(byte(ManualMode));                                                   //1 byte

    SerialUSB.write(byte(lightMeasured%256));SerialUSB.write(byte(lightMeasured>>8)); 
    ...
    */
    ...
    buffer_tx[13] = byte(_EncoderTicks_code>>24);
    buffer_tx[14] = byte(_EncoderTicks_code>>16);
    buffer_tx[15] = byte(_EncoderTicks_code>>8);
    buffer_tx[16] = byte(_EncoderTicks_code%256);
    ...
    if(calculateCRC_tx) {
      ...
      buffer_tx[24] = byte(crc>>8);
      buffer_tx[25] = byte(crc%256);
    }
    else{
      buffer_tx[0] = 0;
      buffer_tx[0] = 0;
  ...
  //Data reception: the data is read at the frequency of the computer
   ...
   if(SerialUSB.available()){
      ...
      if (buffer_rx_ptr == CMD_LENGTH) {
        ...
        liquidLensVAmplitude = float(int(buffer_rx[2]) + int(buffer_rx[3]) *256)/1000*20/1.48;        //the amplitude is sent in mm, and is converted in V: 10V = 1.48 mm
        liquidLensVOffset = float(int(buffer_rx[4]) + int(buffer_rx[5]) *256)/100;
        
        liquidLensOrder=int(buffer_rx[6]);
        
        if (liquidLensVAmplitude>0 && liquidLensOrder==1 ){
          isYtracking=true;
        }
        else{
          isYtracking=false;
        ...
        Homing = int(buffer_rx[7]);                                                                  //Homing and Tracking encoded on the same byte 
        Tracking=int(buffer_rx[8]);
        
        Step_X = float(int(buffer_rx[9])+(int(buffer_rx[10]) << 8));                                   // X steps to move  ATTENTION: We change the sens of the motor 
        if (Step_X>32767){
          Step_X=Step_X-65536;
        }
        Step_X=Step_X/100;                                                                   
         
        Step_Y = float(int(buffer_rx[11])+(int(buffer_rx[12]) << 8));
        if (Step_Y>32767){
          Step_Y=Step_Y-65536;
        }
        Step_Y = Step_Y/100;
        
        Step_Z = float(int(buffer_rx[13])+(int(buffer_rx[14]) << 8)); 
        if (Step_Z>32767){
          Step_Z=Step_Z-65536;
        }
        Step_Z= -Step_Z/100;
        
        DeltaT = float( int(buffer_rx[15])+(int(buffer_rx[16]) << 8))/10;                     // Delta T received in ms 

        lightOutput = int(buffer_rx[17]);              // Intensity sent to the LED
        
//Debugging 
//       SerialUSB.write(byte(int(liquidLensSweepFrequency*100)%256));SerialUSB.write(byte(int(liquidLensSweepFrequency*100)>>8)); 
//       SerialUSB.write(byte(int(liquidLensVAmplitude*1000)%256)); SerialUSB.write(byte(int(liquidLensVAmplitude*1000)>>8));
//       SerialUSB.write(byte(int(liquidLensVOffset*100)%256)); SerialUSB.write(byte(int(liquidLensVOffset*100)>>8));
//       SerialUSB.write(byte(Homing)); 
//       SerialUSB.write(byte(Tracking)); 
//       SerialUSB.write(byte(int(Step_X*100)%256));SerialUSB.write(byte(int(Step_X*100)>>8)); 
//       SerialUSB.write(byte(int(Step_Y*100)%256));SerialUSB.write(byte(int(Step_Y*100)>>8));
//       SerialUSB.write(byte(int(Step_Z*100)%256));SerialUSB.write(byte(int(Step_Z*100)>>8));
//       SerialUSB.write(byte(int(DeltaT*100)%256));SerialUSB.write(byte(int(DeltaT*100)>>8));
        ...
        {
          //XStepperHoming(); //this function will return 1 to the computer
         //YStepperHoming();
          ...
          if(stepperX.distanceToGo()==0)
          {
            ...
            if(moveStart==true)
            {
              ...
              FocusStack(int(nSteps/2)*reachedEnd);
              moveStart = false; 
            }
            else
            {
              FocusStack(nSteps*reachedEnd);
            }
        ...
        if(ManualMode == LOW && Tracking > 0)
        { 
          
//          TargetCurr_X=int(Step_X);
//          TargetCurr_Z=int(Step_Z);

                
          stepperZ.move((int) round(microSteps_Z * Step_Z));
          stepperX.move((int) round(microSteps_X * Step_X));

          // For Focus-tracking experiments the X-stepper behaves like the "hare" which is tracked by the "Y-stepper", so it's not controlled by the computer
//          stepperX.move(/(int) round(microSteps_X * Step_X *((Step_X>0)*_xLimPos + (Step_X < 0) *_xLimNeg)));
          
          if (isYtracking){
//            TargetCurr_Y = int(Step_Y);
//              stepperY.move((int) round(microSteps_Y * Step_Y * ((Step_Y>0)*_yLimPos + (Step_Y < 0) *_yLimNeg))
  ...
  // If the light intensity is > 0 then send a PWM signal to the lightPWM pin
//  currMillisLight = millis() - currMillisLight;
//  if(currMillisLight > lightUpdateTime)
//  {
//    analogWrite(lightPWM, lightOutput);
//    
//  }
...
void YLaunching(){
  // We are using the X-stepper as a stand-in to move the Y-stage that has the prescribed motion
  microSteps_X=16;
  setMS123(microSteps_X,'X');
  stepperSpeedX = 500;
  stepperX.setMaxSpeed(stepperSpeedX); 
...
//                                   Obtain Focus-stack
//-------------------------------------------------------------------------------
void FocusStack(int nSteps){
// Move the X-stepper back and forth while simultaneously obtaining images. 
