...
//--------------------------------------------------
int maxManualSpeedX = 2000, maxManualSpeedY = 2000, maxManualSpeedTheta = 2000, maxManualSpeedZ = 1000; 
//--------------------------------------------------
...
// Set the stage speed of X and Theta stage
int joystickSensitivity_Z,joystickSensitivity_Z_Prev;
...
//  Serial.begin(115200);
  ...
  pinMode(Z_limit_1,INPUT_PULLUP); pinMode(Z_limit_2,INPUT_PULLUP);
  //===============================================================================
  ...
  //===============================================================================
  // Setup Input Start Tracking Trigger Pin
  //===============================================================================
  pinMode(triggerTrack,OUTPUT);
  digitalWrite(triggerTrack,LOW);
  //===============================================================================
  // Setup Indicator Pin
  //===============================================================================
...
void loop()
{
  ...
  ManualMode = digitalRead(ManualPin); // Pin is LOW when Manual and HIGH when Auto
  readLimitSwitch();
  ...
  currMillisStage = millis() - currMillisStage;
   if(currMillisStage >= ManualSampleTime  && StageLocked == false)
    {      
      ...
      joystickSensitivity = analogRead(analogSpeedX);
  ...
  //Data reception: the data is read at the frequency of the computer
   ...
   if(SerialUSB.available()){
      ...
      if (buffer_rx_ptr == CMD_LENGTH) {
        ...
        Step_Theta = float(int(buffer_rx[13])+(int(buffer_rx[14]) << 8)); 
        if (Step_Theta > 32767){
