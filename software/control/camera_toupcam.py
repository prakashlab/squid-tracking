...
class Camera(object):
    ...
    def __init__(
        ...
        self.OffsetX = CAMERA.ROI_OFFSET_X_DEFAULT
        self.OffsetY = CAMERA.ROI_OFFSET_X_DEFAULT
        self.Width = CAMERA.ROI_WIDTH_DEFAULT
        self.Height = CAMERA.ROI_HEIGHT_DEFAULT

        self.WidthMax = CAMERA.ROI_WIDTH_DEFAULT
        self.HeightMax = CAMERA.ROI_HEIGHT_DEFAULT
    ...
    def _update_buffer_settings(self):
        ...
        if (self.data_format == "RGB") & (self.pixel_size_byte != 4):
            bufsize = self._TDIBWIDTHBYTES(width * self.pixel_size_byte * 8) * height
        else:
